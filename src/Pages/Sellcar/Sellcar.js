...
const Sellcar = ({ setPage }) => {
  ...
  const gotoReconfirm = () => {
    ...
    const driving_distance = localStorage.getItem(
      `${carNumber}_driving_distance`
    );
    const additional_info = localStorage.getItem(
      `${carNumber}_additional_info`
    );
  ...
  return (
    <>
      ...
      <Wrap>
        ...
        {Boolean(modal) ? (
          <WarningModal modalText={modal} setModal={setModal} />
        ) : null}
      </Wrap>
    </>
  );
